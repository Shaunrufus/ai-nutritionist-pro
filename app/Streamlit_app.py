...
with st.sidebar:
    ...
    <div style="text-align:center;padding:1.2rem 0 0.5rem">
        ...
        <div style="color:rgba(165,180,252,0.4);font-size:0.68rem;letter-spacing:1.5px;text-transform:uppercase;margin-top:3px">
    ...
    if selected_nav == "🥗 AI Meal Planner":
        ...
        st.markdown(f"""
        <div style="background:rgba(6,182,212,0.05);border:1px solid rgba(6,182,212,0.12);
                    border-radius:10px;padding:0.65rem 0.9rem;margin:0.5rem 0 0.8rem;
                    font-size:0.74rem;color:rgba(165,180,252,0.5)">
            Free plan &nbsp;•&nbsp; All AI queries are private
        </div>
        """, unsafe_allow_html=True)
...
for title, col in nav_bar_items:
    with col:
        ...
        if st.button(title, type=btn_type, use_container_width=True, key=f"top_tab_{title}"):
            ...
            # Do NOT directly set sidebar_nav_radio here — it's a widget key
            # and setting it after render causes StreamlitWidgetAlreadyInstantiatedError.
            # The sidebar radio syncs on next rerun via the index= parameter.
            st.rerun()
...
# ===== 12. FOOTER =====
st.markdown("""
<div style="margin-top:4rem;padding:1.5rem 0;text-align:center;
            border-top:1px solid rgba(255,255,255,0.04)">
    <div style="color:rgba(165,180,252,0.25);font-size:0.72rem;letter-spacing:0.8px">
        © 2025 AI Nutritionist Pro &nbsp;•&nbsp; All data stored privately
    </div>
</div>
""", unsafe_allow_html=True)
